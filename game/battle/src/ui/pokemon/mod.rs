...
impl Default for Spawner {
    fn default() -> Self {
        Self {
            ...
            x: 0.0,
            texture: unsafe { *POKEBALL.get_or_insert(game::graphics::byte_texture(include_bytes!("../../../assets/thrown_pokeball.png"))) }
        }
    }
}
...
impl PokemonRenderer {
    ...
    pub fn with(index: BattleGuiPositionIndex, pokemon: &PokemonInstance, side: PokemonTexture) -> Self {
        Self {
            texture: Some(pokemon_texture(&pokemon.pokemon.value().data.id, side)),
            ..Self::new(index, side)
        }
    }
    ...
    pub fn new_pokemon(&mut self, pokemon: Option<&PokemonInstance>) {
        self.texture = pokemon.map(|pokemon| pokemon_texture(&pokemon.pokemon.value().data.id, self.side));
        self.reset();
    }

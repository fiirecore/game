...
use game::{
	deps::{borrow::BorrowableMut, UNKNOWN16},
...
use crate::{GameBattleWrapper, BattlePlayerGuiRef};
...
impl BattleManager {
	...
	pub fn new(ctx: &mut Context, party: Rc<PartyGui>, bag: Rc<BagGui>) -> BattleManager {
		...
		BattleManager {
			...
			player: BattlePlayerGuiRef::new(ctx, party, bag, BorrowableMut::Borrowed(&mut data_mut().bag), UNKNOWN16),

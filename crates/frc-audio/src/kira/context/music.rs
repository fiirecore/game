...
impl MusicContext {
    ...
    pub fn get_music_playing(&self) -> Result<Music, AudioError> {
        if let Some(current_music) = self.current_music.try_read() {
            if let Some(ref instance) = *current_music {
                return Ok(instance.0);
            } else {
                return Err(AudioError::Missing);
            }
        } else {
            return Err(AudioError::Inaccessable("Could not unlock read for current music RwLock!".to_string()));
        }
    }

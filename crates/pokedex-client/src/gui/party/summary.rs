...
use crate::pokedex::pokemon::{owned::OwnedPokemon, data::PokemonTexture};
...
impl SummaryGui {
    ...
    pub fn ui(&mut self, textures: &PokemonTextures, egui: &egui::Context, party: &[OwnedPokemon]) -> Option<()> {
        if self.alive() {
            egui::Window::new("Summary")
                .show(egui, |ui| {
                    ...
                    ui.separator();
                    let pokemon = &party[self.pokemon];
                    match self.page {
                        0 => {
                            if let Some(id) = textures.get_egui(&pokemon.pokemon.id, PokemonTexture::Front) {
                                ui.image(id, PokemonTextures::SIZE);
                            }
                        ...
                        2 => {
                            egui::Grid::new("Summary Moves").show(ui, |ui| {
                                for m in pokemon.moves.iter() {
                                    ui.label(&m.m.name);
                                    ui.label(format!("PP: {}/{}", m.pp, m.m.pp, ));
                                    ui.end_row();
                                    ui.separator();
                                    ui.end_row();
                                }

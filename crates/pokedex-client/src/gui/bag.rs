...
use crate::{pokedex::item::{bag::OwnedBag, ItemId}, texture::ItemTextures};
...
impl BagGui {
    ...
    pub fn ui(&mut self, textures: &ItemTextures, egui: &egui::Context, bag: &mut OwnedBag) -> Option<BagAction> {
        if self.alive {
            egui::Window::new("Bag").show(egui, |ui| {
                egui::Grid::new("Items").show(ui, |ui| {
                    for stack in bag.iter().filter(|stack| stack.count != 0) {
                        let id = textures
                            .get_egui(&stack.item.id)
                            .copied()
                            .unwrap_or_default();
                        if ui.add(egui::ImageButton::new(id, ItemTextures::SIZE)).clicked() {

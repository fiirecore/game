...
use world::{serialized::SerializedNpcGroup, character::npc::group::{NpcGroupId, NpcGroup}};
...
pub fn load_npc_types(root_path: &Path) -> HashMap<NpcGroupId, SerializedNpcGroup> {
    ...
    .unwrap_or_else(|err| panic!("Could not get warp file at {:?} with error {}", npc_types, err))
        ...
        if path.is_dir() {
            ...
            let group: NpcGroup = ron::from_str(
            ...
            let id1 = id.parse().unwrap_or_else(|err| panic!("Cannot parse npc group file name {} into id with error {}", id, err));

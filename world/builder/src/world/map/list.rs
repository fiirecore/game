...
use super::LoadMapError;

// pub fn load_map_list(root_path: &Path, list: SerializedMapList) -> Result<Vec<WorldMap>, LoadMapError> {
...
//                     "toml" => Some(toml::from_str::<MapConfig>(&read_to_string(&file)?)?),
//                     _ => None,
...
fn serde_err(err: impl std::error::Error, file: &std::path::PathBuf) -> ! {
    panic!(
        "Could not deserialize map set configuration at {:?} with error {}",
        file, err
    )
}

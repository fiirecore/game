...
use worldlib::{
    ...
    map::{
        ...
        ActiveNpc, TileId, World, WorldMap,
    },
    positions::{Direction, Location},
    script::world::{Condition, ScriptWarp, WorldActionKind, WorldScript},
};

use crate::world::{
    battle::{trainer_battle, wild_battle},
    gui::TextWindow,
    npc::npc_type,
...
impl GameWorld for WorldMap {
    fn on_start(&mut self, ctx: &mut Context, music: bool) {
        ...
        if music {
            if audio::music::get_current_music()
                .map(|current| current != self.music)
                .unwrap_or(true)
            {
    ...
    fn on_tile(&mut self, world: &mut WorldMapManagerData, battle: BattleEntryRef) {
        if let Some(tile_id) = self.tile(world.player.character.position.coords) {
            ...
            // look for player
            for (index, npc) in self.npcs.list.iter_mut().filter(|(_, npc)| {
                npc.as_ref()
                    .map(|npc| npc.trainer.is_some())
                    .unwrap_or(false)
            }) {
                find_battle(
                    save,
                            ...
                            Condition::Scripts(scripts) => {
                                for script_condition in scripts {
                                    if save
                                        .world
                                        .scripts
                                        .contains(&script_condition.identifier)
                                        .ne(&script_condition.happened)
                                    {
                                        break_script = true;
                                    }
                                }
    ...
    fn update(
        ...
        if pressed(ctx, Control::A) && self.npcs.active.is_none() {
            let pos = if self
                .tile(world.player.character.position.coords)
                ...
                .unwrap_or_default()
            {
                world
                    ...
                    .character
                    .position
                    .in_direction(world.player.character.position.direction)
            } else {
        ...
        for npc in self.npcs.list.values_mut().flatten().filter(|npc| {
            npc.character.destination.is_some() && npc.movement != MovementType::Still
        }) {
                        ...
                        if NPC_RANDOM.gen_float() < NPC_MOVE_CHANCE {
                            match npc.movement {
                                ...
                                MovementType::WalkUpAndDown(steps) => {
                                    ...
                                    if can_move(
                                        npc.character.move_type,
                                        self.movements[npc.character.position.coords.x as usize
        ...
        for script in self.scripts.iter_mut().filter(|script| script.alive()) {
            match &mut script.current {
                        ...
                        if match &action {
                            ...
                            WorldActionKind::PlayerGivePokemon(instance) => {
                                if let Err(err) = data_mut().party.try_push(instance.clone()) {
                                    warn!(
                                        "Could not add {} to player's party with error {}",
                                        instance.pokemon.name, err
                                    );
                                }
                                false
                            }
                            ...
                            WorldActionKind::NpcMove(id, pos) => {
                                if let Some(npc) = self.npcs.get_mut(id) {
                                    ...
                                    warn!(
                                        "Npc script tried to move an unknown Npc (with id {})",
                                        id
                                    );
                                    false
                                }
                            ...
                            WorldActionKind::NpcLeadPlayer(id, pos) => {
                                if let Some(npc) = self.npcs.get_mut(id) {
                                    ...
                                    if world.player.character.position.coords.ne(&pos.coords) {
                                        world.player.character.destination =
                                            npc.character.destination.clone();
                                        if let Some(destination) =
                                            world.player.character.destination.as_mut()
                                        {
                                            destination.queued_movements.pop_back();
                                            destination.queued_movements.push_front(
                                                world
                                                    .player
                                                    .character
                                                    .position
                            ...
                            WorldActionKind::NpcMoveToPlayer(id) => {
                                if let Some(npc) = self.npcs.get_mut(id) {
                                    npc.character
                                        .go_next_to(world.player.character.position.coords);
                                    true
                                } else {
                            ...
                            WorldActionKind::NpcSay(id, pages) => {
                                if let Some(npc) = self.npcs.get_mut(id) {
                                    ...
                                    process_messages(&mut pages, data());
                ...
                Some(current) => match current {
                    ...
                    WorldActionKind::NpcMove(id, ..) => match self.npcs.get_mut(id) {
                        Some(npc) => {
                    ...
                    WorldActionKind::NpcLeadPlayer(id, ..) => match self.npcs.get_mut(id) {
                        Some(npc) => {
                    ...
                    WorldActionKind::NpcMoveToPlayer(id) => match self.npcs.get_mut(id) {
                        Some(npc) => {
                            if npc.character.move_to_destination(delta) {
                                script.current = None;
                            }
                    ...
                    WorldActionKind::Wait(remaining) => {
                        *remaining -= delta;
                ...
                if npc.character.destination.is_some() {
                    ...
                    match &npc.interact {
                        NpcInteract::Message(pages) => {
                            ...
                            window
                                .text
                                .color(super::npc::npc_type(&npc.type_id).text_color);
                            message_ran = true;
                        }
                                ...
                                if let Some(trainer_type) = npc_type.trainer.as_ref() {
                                    ...
                                    window.text.set(
                                        trainer
                                            .encounter_message
                                            .iter()
                                            .map(|message| MessagePage::new(message.clone(), None))
                                            .collect(),
                                    );
                                    ...
                                    if let Some(encounter_music) = trainer_type.music.as_ref() {
                                        if let Some(playing_music) =
                                            audio::music::get_current_music()
                                        {
                                            if let Some(music) =
                                                audio::music::get_music_id(encounter_music)
                                                    .flatten()
                                            {
                    ...
                    world.player.character.position.direction =
                    ...
                    if !message_ran {
                        ...
                        process_messages(&mut window.text.message.pages, data());
                    }
                }
            }
        }
...
fn debug_input(ctx: &Context, map: &mut WorldMap) {
    if debug_pressed(ctx, DebugBind::F8) {
        for (index, npc) in map
            .npcs
            .list
            .iter()
            .flat_map(|(id, npc)| npc.as_ref().map(|npc| (id, npc)))
        {
            info!(
...
fn find_battle(
    ...
    id: NpcId,
    npc: &mut Option<Npc>,
    active: &mut ActiveNpc,
    player: &mut PlayerCharacter,
) -> bool {
    if !save.world.has_battled(map, &id) {
        if npc
            .as_mut()
            .map(|npc| npc.find_character(&mut player.character))
            .unwrap_or_default()
        {

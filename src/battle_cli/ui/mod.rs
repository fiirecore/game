...
use crate::battle::pokemon::BattlePlayer;

use super::clients::gui::transition::{introduction::BattleIntroductionManager, opener::BattleOpenerManager};
...
pub fn battle_party_known_gui<ID: Sized + Copy + core::fmt::Debug + core::fmt::Display + Eq + Ord>(gui: &PartyGui, party: &BattlePartyKnown<ID>, exitable: bool) {
...
pub fn battle_party_gui<ID: Sized + Copy + core::fmt::Debug + core::fmt::Display + Eq + Ord>(gui: &PartyGui, player: &BattlePlayer<ID>, exitable: bool) {
    gui.spawn(player.party.cloned().into_iter().map(|instance| PokemonDisplay::new(std::borrow::Cow::Owned(instance))).collect(), Some(false), exitable);
}
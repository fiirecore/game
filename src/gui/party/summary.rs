...
use atomic::Atomic;
use pokedex::texture::{pokemon_texture, PokemonTexture::Front};

use crate::input::{pressed, Control};

use crate::graphics::{
    byte_texture, draw_circle, draw_line, draw_rectangle, draw_text_left, position,
};
    ...
    pub fn new(ctx: &mut Context) -> Self {
        Self {
            ...
            pages: [
                byte_texture(
                    ctx,
                    include_bytes!("../../../assets/gui/party/summary/info.png"),
                ),
                byte_texture(
                    ctx,
                    include_bytes!("../../../assets/gui/party/summary/skills.png"),
                ),
                byte_texture(
                    ctx,
                    include_bytes!("../../../assets/gui/party/summary/moves.png"),
                ),
            ],
            offset: Default::default(),
            pokemon_background: byte_texture(
                ctx,
                include_bytes!("../../../assets/gui/party/summary/pokemon.png"),
            ),
            ...
            match self.page.load(Relaxed) {
                0 => {
                    ...
                    for (index, display) in pokemon.types.iter().enumerate() {
                        ...
                        crate::graphics::draw_text_center(
                            ctx,
                            &0,
                            &display.name,
                            &TextColor::White,

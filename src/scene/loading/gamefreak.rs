...
impl super::LoadingScene for GamefreakLoadingScene {
	...
	fn render(&self) {
		...
		draw_text_left(1, &format!("A Button is{:?}Key", input::keyboard::KEY_CONTROLS.read().get(&input::Control::A).unwrap()), TextColor::White, 5.0, 5.0);
		draw_text_left(1, &format!("B Button is{:?}Key", input::keyboard::KEY_CONTROLS.read().get(&input::Control::B).unwrap()), TextColor::White, 125.0, 5.0);
